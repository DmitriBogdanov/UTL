...
void generate_values(const std::filesystem::path& output_path) {
    ...
    utl::random::default_generator gen(seed);
    Dist                           dist;
...
int main() {
    ...
    generate_values<utl::random::approx_normal_distribution<double>>(directory / "approx_normal.csv");
    generate_values<utl::random::normal_distribution<double>>(directory / "precise_normal.csv");
}